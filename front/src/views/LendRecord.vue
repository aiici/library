  ...
  <div class="home" style="padding: 10px">
    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="status" label="状态">
        <template v-slot="scope">
          ...
          <el-tag v-else-if="scope.row.status == 2" type="info">还书中</el-tag>
      ...
      <el-table-column v-if="user.role === 1" label="操作" width="230px">
        <template v-slot="scope">
          <el-button size="mini" @click="handleEdit(scope.row)"
                     :disabled="scope.row.status == 0 || scope.row.status == 1">编辑
          </el-button>
    ...
    <div style="margin: 10px 0">
      ...
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="save(form.readerId,form.isbn)">确 定</el-button>
      </span>
        </template>
      </el-dialog>
    </div>
  ...
  methods: {
    ...
    save(readerId,isbn) {
      ...
      if (this.form.status == 1){
        request.post("/bookwithuser/deleteByLendRecord",this.form).then(res => {
