    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" >
        <template v-slot="scope">
          ...
          <el-button  size="mini" @click ="handlelend(scope.row.id,scope.row.isbn,scope.row.name,scope.row.borrownum)" v-if="user.role == 2" :disabled="scope.row.status == 0 || scope.row.status == 2">借阅</el-button>
