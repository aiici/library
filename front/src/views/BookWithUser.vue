    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed="right" label="操作">
        <template v-slot="scope">
          ...
          <el-popconfirm title="确认还书?" @confirm="handlereturn(scope.row.isbn,scope.row.lendTime)"
                         v-if="user.role == 2" :disabled="scope.row.book.status == 1 || scope.row.book.status == 2">-->
            <template #reference>
              <el-button type="primary" size="mini"
                         :disabled="scope.row.book.status == 1 || scope.row.book.status == 2">还书
              </el-button>
            </template>
          </el-popconfirm>
          <el-popconfirm title="确认续借(续借一次延长30天)?" @confirm="handlereProlong(scope.row)" v-if="user.role == 2"
            ...
            <template #reference>
              <el-button type="danger" size="mini" :disabled="scope.row.prolong == 0 || scope.row.book.status == 1 || scope.row.book.status == 2">续借</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handlereturn(isbn,lendTime){
      this.form.isbn=isbn
      this.form.lendTime=lendTime
      console.log(lendTime)
      this.form.status = "2"
      request.put("/book",this.form).then(res =>{
        request.put("/LendRecord/updateStatus",this.form).then(res =>{
          if(res.code == 0){
            ElMessage({
              message: '已提交还书申请',
              type: 'success',
